...
self.logger.info(self._mask_sensitive_data(str(message)))
def error(self, message):
self.logger.error(self._mask_sensitive_data(str(message)))
def mask_secret(secret, visible_chars=4):
if not secret or len(secret) &lt;= visible_chars * 2:
return &quot;***MASKED***&quot;
return secret[:visible_chars] + &quot;*&quot; * (len(secret) - visible_chars * 2) + secret[-
visible_chars:]
def secure_log_decorator(func):
@wraps(func)
def wrapper(*args, **kwargs):
secure_logger = SecureLogger(func.__module__)
safe_kwargs = {k: (mask_secret(str(v)) if any(x in k.lower() for x in
[&#39;password&#39;, &#39;key&#39;, &#39;token&#39;, &#39;secret&#39;]) else v) for k, v in kwargs.items()}
secure_logger.info(f&quot;Executando {func.__name__} com args:
{safe_kwargs}&quot;)
try:
